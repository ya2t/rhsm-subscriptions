...
public abstract class BaseSnapshotRoller {
    ...
    private boolean updateTotals(boolean override, TallySnapshot snap, ProductUsageCalculation calc,
        ...
        if (override || mustUpdate(measurement.getCores(), calc.getTotalCores())) {
            measurement.setCores(calc.getTotalCores());
            changed = true;
        }

        if (override || mustUpdate(measurement.getSockets(), calc.getTotalSockets())) {
            measurement.setSockets(calc.getTotalSockets());
            changed = true;
        }

        if (override || mustUpdate(measurement.getInstanceCount(), calc.getTotalInstanceCount())) {
            measurement.setInstanceCount(calc.getTotalInstanceCount());
            changed = true;
        }
